...
def block1_instructions():
    textstim.text='This block will be divided into 6 parts, each lasting approximately 2.5 minutes, with breaks in between.\
\n\nIn this block, the added noise will sound like hissing. An amplified version of the noise will be presented twice now.\
...
def block2_instructions():
    textstim.text='Block 2 will also be divided into six parts of the same durations as before, with breaks in between.\
\n\nThe added noise here will sound like a click. An amplified version of the click will be played twice now.\
\n\nPress Enter to play the added noise.'
    textstim.draw()
    expwin.flip()
    responsecomplete=False
...
for i in range(3):
    ...
    np.random.shuffle(pairs_wn_counter)
